...
#        self.server = Server(sr=44100, nchnls=self.nchnls, buffersize=512).boot() FL 29/05/17
        self.server = Server(sr=44100, nchnls=self.nchnls, buffersize=512, duplex=0) #FL 29/05/17
        ...
        self.noSound = True # FL 02/10/2017
...
#        numSpk = pref["NUM_SPEAKERS"] FL 29/05/17
    ...
    def unregisterMeter(self):
        self.ampAnal.setFunction(None)

    def changeSnd(self, snd):
        self.player.path = snd
        self.table.path = snd # JR 31 mai 2017
        if snd == EMPTY_AUDIO_FILE: # FL 02/10/2017
            self.noSound = True # FL 02/10/2017
        else: # FL 02/10/2017
            self.noSound = False # FL 02/10/2017
